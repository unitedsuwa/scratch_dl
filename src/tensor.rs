...
pub fn step_func(input: &TensorAtom) -> TensorAtom {
    let result = input.ten.iter().map(|&f| {
        if f > 0. {
            1.
        } else {
            0.
...
mod tests {
    ...
    fn tensoratom_dot() {
        ...
        assert_eq!(dot_result.ten, vec![2.0, 6.0])
    }
    ...
    fn tensor_sigmoid() {
        ...
        assert_eq!(sig_result.ten, vec![0.2689414213699951, 0.7310585786300049, 0.8807970779778823]);
    }
